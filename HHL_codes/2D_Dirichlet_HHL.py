...
for j in range(N - 1):
    for i in range(N - 1):
        row_idx = i + j * (N - 1)
        ii, jj = i + 1, j + 1  
        f_ij = np.sin(np.pi * X[ii, jj]) * np.sin(np.pi * Y[ii, jj]) * (dx ** 2)
        b[row_idx] = f_ij
...
#===== REAL SOLUTION =====
x_real = -(np.sin(np.pi * X) * np.sin(np.pi * Y)) / (2.0 * np.pi ** 2)
...
plt.xlabel('x'); plt.ylabel('y'); plt.title('Relative error (HHL vs exact)')
plt.tight_layout(); plt.show()
