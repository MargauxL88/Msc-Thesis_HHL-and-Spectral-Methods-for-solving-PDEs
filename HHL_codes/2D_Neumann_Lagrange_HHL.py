...
for i in range(N - 1):
    for j in range(N - 1):
        row_idx = i + j * (N - 1)
        ii, jj = i + 1, j + 1
        f_ij = np.cos(np.pi * X[ii, jj]) * np.cos(np.pi * Y[ii, jj]) * (dx ** 2)
        b[row_idx] = f_ij
...
# ===== REAL SOLUTION =====
p_real = -(1.0 / (2.0 * np.pi ** 2)) * np.cos(np.pi * X) * np.cos(np.pi * Y)
...
# ===== PLOTTING =====
print(f"Execution time (HHL): {t_hhl:.4f} s")
print(f"Execution time (Classical): {t_classic:.6f} s")
print(f"Mean relative error = {mean_error:.3f} %")
print("-" * 60)

fig = plt.figure(figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')
ax.plot_wireframe(X, Y, p_exp, color='r', label='HHL (rescaled)')
ax.plot_wireframe(X, Y, p_real, color='b', linestyle='dashed', label='Exact')
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('p(x,y)')
...
plt.xlabel('x')
plt.ylabel('y')
plt.title('Error grid (HHL vs exact)')
plt.tight_layout()
plt.show()
